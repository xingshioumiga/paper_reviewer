...
def main() -> None:
    ...
    max_no_improve = args.max_no_improve or int(config.get("max_no_improve", 2))
    ...
    logger.info(
        "run start: input=%s output=%s max_iterations=%s max_no_improve=%s log_file=%s",
        test_file,
        output_path,
        max_iterations,
        max_no_improve,
    ...
    logger.info(
        "run complete: iterations=%s best_score=%.4f history_items=%s output_file=%s",
